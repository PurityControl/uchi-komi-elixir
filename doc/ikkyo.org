...
- [[https://nodejs.org/en/][NodeJS]] or node through [[https://github.com/creationix/nvm][NVM]]
  - for nvm ensure
    - correct shell file has the config: nvm uses ~/.bashrc by default but mac
      uses ~/.bash_profile when opening a shell
    - set a default nodejs to launch wheneve a shell is opened
      - ~nvm install 5.0~
...
you will need to install Erlang (and Elixir's) package manage hex.
...
#+BEGIN_SRC bash
mix archive.install https://github.com/phoenixframework/phoenix/releases/
